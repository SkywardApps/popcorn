...
- (pending) Add `.github/workflows/tests.yml`: runs `dotnet test` on both Popcorn.FunctionalTests + Popcorn.SourceGenerator.Tests on PR + push. Closes the "tests don't run on CI" gap.
